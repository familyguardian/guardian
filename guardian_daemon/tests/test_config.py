...
def test_config_validation_invalid_time_format():
    ...
    for invalid_time in invalid_times:
        ...
        try:
            with pytest.raises(ConfigError) as exc_info:
                Config(temp_path)
            assert "time format" in str(exc_info.value).lower() or "must be a string" in str(exc_info.value)
        finally:
            os.unlink(temp_path)
...
def test_config_validation_invalid_grace_minutes():
    ...
    with tempfile.NamedTemporaryFile(mode='w', suffix='.yaml', delete=False) as f:
        config = {
            ...
            "users": {
                "testuser": {
                    "grace_minutes": -5  # Negative grace minutes
                }
            }
        }
        yaml.dump(config, f)
    ...
    try:
        with pytest.raises(ConfigError) as exc_info:
            Config(temp_path)
        assert "grace_minutes" in str(exc_info.value) and ("must be positive" in str(exc_info.value) or "must be non-negative" in str(exc_info.value))
    finally:
        os.unlink(temp_path)
