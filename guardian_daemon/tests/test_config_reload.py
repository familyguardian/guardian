...
async def test_config_reload_rolls_back_on_error(mock_daemon):
    ...
    mock_daemon.usermanager.write_time_rules = MagicMock(side_effect=Exception("Write failed"))
    
    # Mock hash to trigger reload
    with patch.object(mock_daemon, '_get_config_hash', side_effect=["hash1", "hash2", "hash2"]):
        # Run one iteration
        task = asyncio.create_task(mock_daemon.periodic_reload())
        await asyncio.sleep(0.1)
        task.cancel()
