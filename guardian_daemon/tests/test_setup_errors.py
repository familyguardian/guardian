...
def mock_policy():
    ...
    policy = MagicMock()
    policy.data = {"users": {"testuser": {"quota": {"daily": 120}, "monitored": True}}}
    return policy
    ...
    with patch("grp.getgrnam", side_effect=KeyError("Group not found")):
        ...
        with patch(
            ...
            side_effect=subprocess.CalledProcessError(
                1, "groupadd", stderr="Permission denied"
            ),
        ):
...
def test_setup_error_on_group_creation_timeout(mock_policy):
    ...
    with patch("grp.getgrnam", side_effect=KeyError("Group not found")):
        ...
        with patch(
            "subprocess.run", side_effect=subprocess.TimeoutExpired("groupadd", 5)
        ):
            with pytest.raises(SetupError) as exc_info:
                manager.ensure_kids_group()
